...
class McpToolHandlers:
    ...
    def kb_gc_evaluate(self, kb_id: str, file_paths: list[str] | None = None) -> dict:
        if kb_id not in self.kb_registry:
            raise ValueError(f"Unknown KB id: {kb_id}")

        kb_root = self.kb_registry[kb_id]
        if file_paths:
            candidate_paths = [kb_root / relative_path for relative_path in file_paths]
        else:
